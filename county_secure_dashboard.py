...
def update_rank_display(county_info, comparison_mode):
    if not county_info:
        return ""
    ...
    try:
        ...
        if comparison_mode == 'state':
            result = provider.get_population_rank(county_fips, state_code)
        else:
            result = provider.get_population_rank(county_fips, None)

        if result:
            pop_formatted = f"{result['population']:,}"
            return html.Span([
                html.Span("Population: ", className="font-medium text-gray-600"),
                html.Span(
                    f"{pop_formatted} ({result['rank']} of {result['total']} counties)",
